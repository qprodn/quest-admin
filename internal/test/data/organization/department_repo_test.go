...
func TestDepartmentRepo_FindByName(t *testing.T) {
	ctx := context.Background()
	mockRepo := new(MockDepartmentRepo)
	...
	mockRepo.On("FindByName", ctx, "Engineering").Return(expected, nil)

	result, err := mockRepo.FindByName(ctx, "Engineering")
...
func TestDepartmentRepo_FindByParentID(t *testing.T) {
	ctx := context.Background()
	mockRepo := new(MockDepartmentRepo)

	depts := []*org.Department{
		{ID: "dept-1", Name: "Backend", ParentID: "dept-0"},
		{ID: "dept-2", Name: "Frontend", ParentID: "dept-0"},
	}

	mockRepo.On("FindByParentID", ctx, "dept-0").Return(depts, nil)

	result, err := mockRepo.FindByParentID(ctx, "dept-0")
...
func TestDepartmentRepo_UpdateUpdate(t *testing.T) {
	ctx := context.Background()
	mockRepo := new(MockDepartmentRepo)
	...
	mockRepo.On("Update", ctx, mock.AnythingOfType("*organization.Department")).Return(dept, nil)

	result, err := mockRepo.Update(ctx, dept)
...
func TestDepartmentRepo_Delete(t *testing.T) {
	...
	mockRepo.On("Delete", ctx, "dept-1").Return(nil)

	err := mockRepo.Delete(ctx, "dept-1")
...
func TestDepartmentRepo_FindListByIDs(t *testing.T) {
	ctx := context.Background()
	mockRepo := new(MockDepartmentRepo)
	...
	mockRepo.On("FindListByIDs", ctx, []string{"dept-1", "dept-2"}).Return(depts, nil)

	result, err := mockRepo.FindListByIDs(ctx, []string{"dept-1", "dept-2"})
